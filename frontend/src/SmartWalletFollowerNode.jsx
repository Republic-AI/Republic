...
export default function SmartWalletFollowerNode({ data }) {
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      ...
      try {
        const response = await fetch('http://localhost:5002/fetch-smart-wallet-transactions', {
          method: 'POST',
        ...
        console.error('Error fetching smart wallet transactions:', error);
      }
    };
  ...
  return (
    <div className={`custom-node ${isConfigOpen ? 'expanded' : ''}`}>
      ...
      <div className="node-header">
        <select className="node-type-select" value="smartWalletFollower" disabled>
          <option value="smartWalletFollower">Smart Wallet Follower</option>
        </select>
        <button 
